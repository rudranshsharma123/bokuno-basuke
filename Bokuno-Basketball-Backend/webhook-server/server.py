...
db_params = dict(provider='cockroach',
                 ...
                 port=26257,
                 database='shiny-wolf-1947.defaultdb',
                 password="rudranshsharma123")

app = Flask(__name__)
app.config.update(dict(DEBUG=True, PONY=db_params))
Pony(app)
...
def add_values(userid, searchtext, searchintent, imagelink, searchkeyword):
    Search(user=User.get(user_id=userid),
...
def return_words():
  ...
  """
    ...
    if not active_user:
        ...
        a = select(s.searchKeyWord for s in Search
                   if s.user.user_id == active_user)[:]
        a = list(a)
        return " ".join(a)
...
def MLHhacks():
  ...
  """
    ...
    x = [
        x.text.replace("DIGITAL", "").replace("HIGH SCHOOL", "")
        for x in raw_reponse
    ]
...
def IwantToLearn(learn):
  ...
  """
    ...
    url = "https://www.google.com/search?q=I+want+to+learn+{learn}".format(
        learn=learn)
    from requests_html import HTMLSession
    print(url)
    session = HTMLSession()
...
def try_add_values(userid, searchtext, searchintent, imagelink, searchkeyword):
    ...
    try:
        x = add_values(userid=active_user,
                       searchtext=searchtext,
                       searchintent=searchintent,
                       imagelink="yoImage",
                       searchkeyword=searchkeyword)
...
def webhook():
    ...
    try:
        ...
        if intent == "FindNextMLHHack":
            ...
            # def add_values(userid, searchtext, searchintent, imagelink, searchkeyword):
            text = add_values(userid=active_user,
                              searchtext=searchtext,
                              searchintent=intent,
                              imagelink="yo",
                              searchkeyword=searchKeyword)
            print(text)
            if text == "Not Log In":
                return {"fulfillmentText": return_text, "source": "webhook"}
            else:
                return {"fulfillmentText": return_text, "source": "webhook"}
        ...
        if intent == "IwantToLearn":
            ...
            text = try_add_values(userid=active_user,
                                  searchtext=searchtext,
                                  searchintent=intent,
                                  searchkeyword=searchKeyword,
                                  imagelink="yo")
        ...
        print("def not here")
        return {"fulfillmentText": str(e), "source": 'webhook'}
